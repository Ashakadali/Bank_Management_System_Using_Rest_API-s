...
//	StatementDAO statementDao;
	...
	public ResponseEntity<ResponseStructure<String>> userLogin(String emailid, int pin) {
		...
		List<BankUserDetails> alldetails=bankUserDao.getAllDetails();
		if(bankUserDao.validateUserDetailsByUsingEmailidAndPassword(emailid, pin) && pin!=0)
		{
			ResponseStructure<String> responseStructure=new ResponseStructure<String>();
	...
	@Override
	public ResponseEntity<ResponseStructure<String>> modifyExixstingUser(int id,BankUserDetails bankUserDetails) {
		BankUserDetails detailsByPin = bankUserDao.getUserDetailsByUsingId(bankUserDetails.getId());
		if(bankUserDetails.getName().length()<=2) {
			throw new BankUserException("Invalid name length");
		}
		List<BankUserDetails> allDetails = bankUserDao.getAllDetails();
		allDetails=allDetails.stream().filter(user->user.getId()!=bankUserDetails.getId()).collect(Collectors.toList());
	if(allDetails.stream().anyMatch(user->user.getEmailid().equalsIgnoreCase(bankUserDetails.getEmailid()))) throw new BankUserException("Email Already In use");
	if(bankUserDetails.getMobilenumber()<6000000000l || bankUserDetails.getMobilenumber()>9999999999l) throw new BankUserException("Invalid mobile Number");
	if(allDetails.stream().anyMatch(user->user.getMobilenumber()==bankUserDetails.getMobilenumber())) throw new BankUserException("Mobile already in use");
	detailsByPin.setName(bankUserDetails.getName());
	detailsByPin.setEmailid(bankUserDetails.getEmailid());
	detailsByPin.setMobilenumber(bankUserDetails.getMobilenumber());
	...
	if(userRegistration!=null) {
		...
		structure.setResponsemsg("User Modified");
		return new ResponseEntity<ResponseStructure<String>>(structure,HttpStatus.ACCEPTED);
	}else {
		ResponseStructure<String> structure=new  ResponseStructure<String>();
		structure.setHttpstatuscode(HttpStatus.BAD_REQUEST.value());
		structure.setResponsemsg("Unable to Modify");
